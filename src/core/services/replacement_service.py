...
def replace_document(document: Document, targets: list[ReplacementTarget]) -> Document | ReplacementError:
    """Replace text in PDF, DOCX, or PPTX documents."""
    try:
        ...
        if mime_type not in (PDF_MIME_TYPE, DOCX_MIME_TYPE, PPTX_MIME_TYPE):
            return ReplacementError(message="Only PDF, DOCX, and PPTX documents are currently supported for data replacement")
